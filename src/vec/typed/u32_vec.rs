	...
	Self::BVecType: GBVec,
{
	...
	fn write_to_slice(self, slice: &mut [u32]);
...
impl U32Vec for glam::UVec2 {
	...
	const DIM: usize = 2;
	fn splat(v: u32) -> Self { Self::splat(v) }
	fn from_slice(slice: &[u32]) -> Self { Self::from_slice(slice) }
	fn write_to_slice(self, slice: &mut [u32]) { self.write_to_slice(slice) }
	fn dot(self, rhs: Self) -> u32 { self.dot(rhs) }
	...
	fn min_element(self) -> u32 { self.min_element() }
	fn max_element(self) -> u32 { self.max_element() }
	fn element_sum(self) -> u32 { self.element_sum() }
	fn element_product(self) -> u32 { self.element_product() }
	...
	fn length_squared(self) -> u32 { self.length_squared() }
